...
class M2AverageZone:
    ...
    def run(self):
        ...
        while True:
            ...
            # Identifying Trend
            if current_price > ema_trailing_period and ema_support_low > ema_consolidation_filter and ema_consolidation_filter > ema_long_term_trend:
                trend = 'bullish 🟢'
            elif current_price < ema_trailing_period and ema_resistance_high < ema_consolidation_filter and ema_consolidation_filter < ema_long_term_trend:    
                trend = 'bearish 🟡'
            else:
                trend = 'consolidation 🔵'    
            ...
            # NOTES TABLE
            #--------------------------------                                                                                                       ----------                  
            print(f"Note: Same as strategy_28.py (strategy_02.py)")
            print(f"Difference:")
            print(f"Bullish: current_price > ema_trailing_period (instead of > ema_support_low) ")
...
def start_strategy():
    ...
    config_settings = TradingConfig(
        ...
        filename=filename,
        strategy_id=64 if production_status == 'DEV' else 64 if production_status == 'DEMO'  else 29, # if LIVE
        volume = 0.01 if production_status == 'DEV' else 0.01 if production_status == 'DEMO' else 0.1, # if LIVE
        deviation=20,
        sl_points=300,
        tp_points=600,
        trailing_activation_points=300, # (3500 = 2x ave. candle range in M2) 2000 points or $0.2 profit | 10 = 1000, 20 = 2000
