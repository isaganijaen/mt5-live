...
from modules.mt5_config_v1_2_0 import TradingConfig
...
from modules.position_manager_M15 import PositionManager # Import the new class 
...
# Main Strategy
#-------------------------------------
class M15AverageZone:
    """
    Encapsulates the full logic for the M15 Average Zone Strategy.
    """
    ...
    def get_data(self):
        ...
        """
        rates = mt5.copy_rates_from_pos(self.config.symbol, mt5.TIMEFRAME_M15, 0, 20000)
        if rates is None:
    ...
    def execute_trade(self, order_type, rates_df):
        ...
        try:
            ...
            self.screenshot_tool.create_trade_chart(
                ...
                comment="M15_Average_Zone", # A descriptive comment
    ...
    def run(self):
        ...
        """
        log_info(f"Starting {self.config.symbol} M15 Average Zone Strategy.\n")
        ...
        while True:
            ...
            ema_entry_guide = indicator_tools.get_last_ema_value(
                period=self.config.entry_guide,
                price_type='close'
            )
            ...
            points_distance_vs_entry_guide = abs(current_price - ema_entry_guide) / point
            ...
            config_indicators_table.add_row(f"{self.config.entry_guide} Period EMA Close", str(round(ema_entry_guide,3)), "Entry Guide" )
            ...
            # Identifying Trend
            if (current_price > ema_entry_guide)  and (ema_entry_guide > ema_support_low) and (ema_support_low > ema_consolidation_filter) and(ema_consolidation_filter > ema_long_term_trend):
                trend = 'bullish 🟢'
            elif (current_price < ema_entry_guide)  and (ema_entry_guide < ema_support_low) and (ema_support_low < ema_consolidation_filter) and(ema_consolidation_filter < ema_long_term_trend):    
                trend = 'bearish 🟡'
            else:
                trend = 'consolidation 🔵'    
            ...
            notes_table.add_row("Entry Guide",f"{self.config.entry_guide}","Entry Guide (between S/R)")
            notes_table.add_row("Trail Guide",f"{self.config.trailing_period}","Trailing Indicator  (EMA Close)")
            ...
            notes_table.add_row("NOTE","✨ M15 3ema Low/High 10ema over 21ema TP=1600. Uses 10 EMA as Trailing Guide to lockin profit.","")
            ...
            if trend == 'bullish 🟢' and points_distance_vs_entry_guide <= distance_threshold_in_points:            
                print("Buying!")
                signal = 'buy'
            ...
            # Disabling Candle Range threshold for now as trades would be limited on a trending market.
            ...
            elif trend == 'bearish 🟡' and points_distance_vs_entry_guide <= distance_threshold_in_points:                    
                ...
                if trend == 'bullish 🟢':
                    ...
                    log_info(f"Bullish trend but price's distance is too far from Support Zone/Trailing Guide ({points_distance_vs_entry_guide:.2f} points)")
                    ...
                    log_info(f"Signal: {signal}")
                    log_info(f"No valid trading signal detected.")
                    log_info(f"Bearish trend but price's distance is too far from Resistance Zone/Trailing Guide ({points_distance_vs_entry_guide:.2f} points).")
                    if not h1_within_range:
                        log_info(f"Note: 1H candle range {candle_1h_range} outide the treshold {self.config.max_candle_range_1h_allowed}.")
                    if not h4_within_range:
                        log_info(f"Note: 4H candle range {candle_4h_range} outide the treshold {self.config.max_candle_range_4h_allowed}.")                         
...
def start_strategy():
    ...
    description = 'M15 Average Zone Trading (2R)'
    ...
    my_strategy = M15AverageZone(config=config_settings, mt5_manager=mt5_manager, position_open_event=position_open_event,screenshot_tool=screenshot_tool)
    try:
        my_strategy.run()
