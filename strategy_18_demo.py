...
class M1AverageZone:
    ...
    def run(self):
        ...
        while True:
            ...
            tbl_notes.add_row(f"Direct Predecessor: strategy_17_demo.py\n")
            tbl_notes.add_row(f"Similarities: Super Aggressive 3-Period EMA High/Low/Trail.")
            tbl_notes.add_row(f"Main difference: Indefinite TP ")
            tbl_notes.add_row(f"Expecting a behavior like DEMO_strategy_03 with consistent profitability while trading much actively with no definite TP! ✨.\n")
...
def start_strategy():
    ...
    config_settings = TradingConfig(
        ...
        filename=filename,
        strategy_id=56 if production_status == 'DEMO' else 18, # if live
