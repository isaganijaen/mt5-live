#-----------------------------------------
# filename: strategy_01.py
# description: 2min 20/200 EMA Trend Trading Strategy -  Full Trailing Only
#-----------------------------------------
...
from modules.trading_hours_01am_to_04am_10am_to_17pm import is_trading_hours
...
strategy_id = 97
strategy_name = '2min 20/200 EMA Trend Trading Strategy -  Full Trailing Only'
trade_note = '2025-10-01 12:47 AM -  PRE-LIVE | 2025-09-10 08:00 PM - Previously know as strategy_02_02.py - Added H1 and H4 Candle Size Range Check - Fixed Trailing Stop Logic'
...
# Risk management
SL_POINTS = 300
TP_POINTS = 2700 #(This is just a placeholder. 'tp' property is ommitted from the request{} request.
...
def calculate_emas(data):
    ...
    ema_trailing_stop = talib.EMA(close_prices, timeperiod=EMA_TRAILING)
    ...
    latest_ema_trailing_stop = ema_trailing_stop[~np.isnan(ema_trailing_stop)][-1] if ema_trailing_stop[~np.isnan(ema_trailing_stop)].size > 0 else None
    
    if any(ema is None for ema in [latest_ema_20, latest_ema_200, latest_ema_trailing_stop]):
    ...
    result = {
        ...
        "ema_trailing_stop": latest_ema_trailing_stop,
        "current_price": current_price
    }
    
    logging.info(f"EMAs calculated - Price: {current_price:.2f}, 20EMA: {latest_ema_20:.2f}, 200EMA: {latest_ema_200:.2f}, 21EMA: {latest_ema_trailing_stop:.2f}")
    return result
...
def manage_trailing_stops():
    ...
    while not stop_trailing:
        try:
            ...
            point = symbol_info.point
            ema_trailing_stop = indicators['ema_trailing_stop']
            ...
            for pos in positions:
                try:
                    ...
                    if pos.type == mt5.ORDER_TYPE_BUY:
                        ...
                        new_sl = ema_trailing_stop - (TRAILING_OFFSET_POINTS * point)
                        # Only move SL up
                        if profit_points >= TRAILING_TRIGGER_POINTS and new_sl > pos.sl:
                            new_sl = round(new_sl, symbol_info.digits)
                            modify_position_sl(pos.ticket, new_sl, pos.tp)
                            logging.info(f"Trailed BUY SL to {new_sl:.2f} (21EMA: {ema_trailing_stop:.2f})")
                        ...
                        if profit_points >= TRAILING_TRIGGER_POINTS and new_sl < pos.sl:
                            new_sl = round(new_sl, symbol_info.digits)
                            modify_position_sl(pos.ticket, new_sl, pos.tp)
                            logging.info(f"Trailed SELL SL to {new_sl:.2f} (21EMA: {ema_trailing_stop:.2f})")
