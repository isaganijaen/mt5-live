...
class M1AverageZone:
    ...
    def run(self):
        ...
        while True:
            ...
            # Identifying Trend
            if current_price > ema_consolidation_filter and ema_consolidation_filter > ema_support_low and ema_support_low > ema_long_term_trend:
                trend = 'bullish 🟢'
            elif current_price < ema_consolidation_filter and ema_consolidation_filter < ema_resistance_high and ema_resistance_high < ema_long_term_trend:    
                trend = 'bearish 🟡'
            else:
                trend = 'consolidation 🔵'    
...
def start_strategy():
    ...
    config_settings = TradingConfig(
        ...
        filename=filename,
        strategy_id=74 if production_status == 'DEMO'  else 39, # if LIVE
        volume = 0.01 if production_status == 'DEV' else 0.01 if production_status == 'DEMO' else 0.1, # if LIVE
        deviation=20,
        sl_points=300,  
        tp_points=300,
        trailing_activation_points=290, # (3500 = 2x ave. candle range in M1) 2000 points or $0.2 profit | 10 = 1000, 20 = 2000
