...
class M1AverageZone:
    ...
    def run(self):
        ...
        while True:
            ...
            print(f"Like its predecessor `strategy_09_demo.py` . It uses the same settings where 7 EMA CLOSE is its Entry Zone.")
            print(f"The only difference is that the old one has SL=300 instead of SL=250 like this.")
...
def start_strategy():
    ...
    config_settings = TradingConfig(
        ...
        filename=filename,
        strategy_id=46 if production_status == 'DEMO' else 10, # if live
